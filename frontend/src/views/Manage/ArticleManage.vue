        ...
        <el-row type="flex" justify="center" align="middle">
            ...
            <el-col :span="6">
                <el-radio-group v-model="selectBy">
                    <el-radio-button label="帖子id" />
                    <el-radio-button label="帖子标题" />
                    <el-radio-button label="帖子内容" />
                </el-radio-group>
            </el-col>
        </el-row>
        ...
        <div id="result">
            <div v-for="result of results" :key="result.uid">
                ...
                <el-row style="margin-bottom: 1em;" type="flex" justify="center" align="middle">
                    <el-col :span="15">
                        <ArticleTab :article="result"></ArticleTab>
                        <!-- <span>
                    ...
                    <el-col :span="2">
                        <el-popover :visible="deletePannelVisible" placement="top" :width="160">
                            ...
                            <template #reference>
                                <el-button @click="deletePannelVisible = true" type="danger">删除帖子</el-button>
                            </template>
                        </el-popover>
                    </el-col>
                    <el-divider></el-divider>
...
const search = async () => {
    results.length = 0;
    console.log(results)
    if (selectBy.value == '帖子id') {
        await searchById();
    } else if (selectBy.value == '帖子标题') {
        await searchByTitle();
    } else if (selectBy.value == '帖子内容') {
...
const searchById = async () => {
    await axios.get(restServerSocket + '/manage/article_by_id', {
...
const searchByTitle = async () => {
    await axios.get(restServerSocket + '/manage/article_by_title', {
...
const searchByContent = async () => {
    await axios.get(restServerSocket + '/manage/article_by_content', {
...
const deleteItem = async (item) => {
    await axios.delete(restServerSocket + '/manage/article', {
        params: { id: item.id }
    }).then(resp => resp.data)
        .then(data => {
            if (data.status == 200) {
